...
*Current status*: public Dataset v2 is live at `taeyangg4/koneps-public-procurement-intelligence`. Kaggle live readback confirms `ready` status, the seven canonical Parquet files plus `00_quickstart_tender_summary.csv` (470,937 rows, 29 columns), 2,767,651,843 total bytes, the custom cover, source provenance, `other` license, valid tags, monthly update frequency, and Usability 10/10. Data Explorer descriptions are present and exact against repository metadata for all 8 files and all 160 parsed columns. The starter overview notebook is public at `taeyangg4/koneps-procurement-5-minute-market-overview`; v4 repaired the Korean category mapping, added the single-file quickstart path, and executed successfully on Kaggle. Remaining content work is the leak-free ML baseline, supplier/market-structure notebook, and automated incremental version update.
